...
Upload/delete/private-read는 Track A의 canonical `src/auth-v1.ts` `requireActor()`를 사용한다. Bearer JWT는 Track A의 Neon Auth JWKS/issuer/audience 검증과 동일한 product actor bridge를 통과하며, storage route에는 별도의 `actorFromRequest` 또는 `AUTH_ADAPTER_PENDING` 경로가 없다. `APP_ENV=production`에서는 Track A와 동일하게 dev-header bypass가 허용되지 않는다.
...
- backend contract test
  ...
  - canonical Track A auth resolver assertion
