...
async function listCodes(
  ...
  const rows = await sql`
    ...
    order by
      case when cu.building_code ~ '^[0-9]+
      cu.building_code asc,
      case when cu.unit_code ~ '^[0-9]+
      cu.unit_code asc
  `;
  return ok({
...
export async function handleAdminHouseholdCodeRequest(
 ...
 then cu.building_code::numeric else null end nulls last,
      cu.building_code asc,
...
export async function handleAdminHouseholdCodeRequest(
 ...
 then cu.unit_code::numeric else null end nulls last,
      cu.unit_code asc
  `;
  return ok({
    households: rows.map((row) => ({
