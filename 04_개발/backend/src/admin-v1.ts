...
async function applicationContext(sql: Sql, applicationId: string) {
  const rows = await sql`
    select a.id, a.status, a.approved_business_id, a.category_name, c.slug as complex_slug
...
async function patchApplication(request: Request, sql: Sql, actor: Actor, applicationId: string, requestId: string) {
  ...
  const manager = await requireManager(sql, actor.id, String(current.complex_slug), requestId);
...
async function createBenefit(request: Request, sql: Sql, actor: Actor, complexSlug: string, requestId: string) {
  ...
  const related = await sql`
    ...
    where b.id = ${businessId}::uuid and r.complex_id = ${String(manager.complex_id)}::uuid
    limit 1
  `;
...
export async function handleAdminRequest(request: Request, env: CoreEnv, requestId: string): Promise<Response | null> {
  ...
  if (match && request.method === 'PATCH') return patchApplication(request, sql, actor, match[1], requestId);

  match = path.match(/^\/api\/v1\/admin\/complexes\/([^/]+)\/posts$/);
  if (match && request.method === 'POST') return createPost(request, sql, actor, decodeURIComponent(match[1]), requestId);

  match = path.match(/^\/api\/v1\/admin\/posts\/([0-9a-fA-F-]+)$/);
  if (match && request.method === 'PATCH') return patchPost(request, sql, actor, match[1], requestId);

  match = path.match(/^\/api\/v1\/admin\/complexes\/([^/]+)\/benefits$/);
  if (match && request.method === 'POST') return createBenefit(request, sql, actor, decodeURIComponent(match[1]), requestId);

  match = path.match(/^\/api\/v1\/admin\/benefits\/([0-9a-fA-F-]+)$/);
  if (match && request.method === 'PATCH') return patchBenefit(request, sql, actor, match[1], requestId);
