...
async function requireActor(request: Request, env: AdminEnv, sql: Sql, requestId: string): Promise<Actor | Response> {
...
async function requireManagerForSlug(sql: Sql, actorId: string, complexSlug: string, requestId: string) {
  const rows = await sql`
    select c.id as complex_id, c.slug, c.name, m.role, m.verification_status
...
async function complexSlugForApplication(sql: Sql, applicationId: string) {
  const rows = await sql`
    ...
    sql`
      ...
      created_media as (
        ...
        select
          ...
          a.business_name || ' 대표 이미지',
          0
        from approved a
        where nullif(trim(a.representative_image_object_key), '') is not null
          and not exists (
            select 1 from business_media bm
            where bm.business_id = a.approved_business_id
              and bm.object_key = a.representative_image_object_key
          )
        returning id
      ),
      created_benefit as (
        insert into benefits (
          complex_id, business_id, title, description, conditions, status
        )
        select
          a.complex_id,
          a.approved_business_id,
...
async function patchApplication(
  ...
  const current = await complexSlugForApplication(sql, applicationId);
  if (!current) return fail('NOT_FOUND', 'Business application not found', 404, requestId);

  const manager = await requireManagerForSlug(sql, actor.id, String(current.complex_slug), requestId);
  ...
  const reviewNoteRaw = String(payload.reviewNote ?? '').trim();
  const reviewNote = reviewNoteRaw || null;
  ...
  if (status === 'approved') {
    if (String(current.status) === 'approved' && current.approved_business_id) {
      return ok({
        id: current.id,
        status: 'approved',
        approvedBusinessId: current.approved_business_id,
        alreadyApproved: true
      }, requestId);
    }

    const approved = await approveApplication(sql, actor.id, applicationId, reviewNote);
    if (approved) return ok(approved, requestId);

    const raced = await complexSlugForApplication(sql, applicationId);
    if (raced && String(raced.status) === 'approved' && raced.approved_business_id) {
      return ok({
        id: raced.id,
        status: 'approved',
        approvedBusinessId: raced.approved_business_id,
        alreadyApproved: true
      }, requestId);
    }
  ...
  const latest = await complexSlugForApplication(sql, applicationId);
...
async function createPost(request: Request, sql: Sql, actor: Actor, complexSlug: string, requestId: string) {
  const manager = await requireManagerForSlug(sql, actor.id, complexSlug, requestId);
  if (manager instanceof Response) return manager;
  ...
  const rows = await sql`
    ...
    values (
      ${String(manager.complex_id)}::uuid,
      ${actor.id}::uuid,
      ${sourceName}, ${category}, ${title}, ${body},
      ${String(payload.attachmentObjectKey ?? '') || null},
      ${status},
      case when ${status} = 'published'
        then coalesce(${String(payload.publishedAt ?? '') || null}::timestamptz, now())
        else null end
    )
...
async function patchPost(request: Request, sql: Sql, actor: Actor, postId: string, requestId: string) {
  const rows = await sql`
...
async function createBenefit(request: Request, sql: Sql, actor: Actor, complexSlug: string, requestId: string) {
  const manager = await requireManagerForSlug(sql, actor.id, complexSlug, requestId);
  if (manager instanceof Response) return manager;
  const payload = await bodyJson(request, requestId);
  if (payload instanceof Response) return payload;
  ...
  const visible = await sql`
    ...
    where b.id = ${businessId}::uuid
      and r.complex_id = ${String(manager.complex_id)}::uuid
    limit 1
  `;
  if (!visible[0]) return fail('VALIDATION_ERROR', 'Business is not related to this complex', 400, requestId);
  ...
  const created = await sql`
    insert into benefits (
      ...
      ${String(payload.conditions ?? '') || null},
      ${String(payload.startsAt ?? '') || null}::timestamptz,
      ${String(payload.endsAt ?? '') || null}::timestamptz,
      ${status}
    )
    returning id, business_id, title, description, conditions, starts_at, ends_at, status, created_at
  `;
  return ok(created[0], requestId, 201);
}

async function patchBenefit(request: Request, sql: Sql, actor: Actor, benefitId: string, requestId: string) {
  const rows = await sql`
  ...
  const startsAt = payload.startsAt === undefined
    ? (current.starts_at ? String(current.starts_at) : null)
    : (String(payload.startsAt).trim() || null);
  const endsAt = payload.endsAt === undefined
    ? (current.ends_at ? String(current.ends_at) : null)
    : (String(payload.endsAt).trim() || null);
...
export async function handleAdminRequest(
  ...
  if (match && request.method === 'GET') {
    ...
    const allowed = ['draft','pending','changes_requested','approved','rejected'];
    if (status && status !== 'all' && !allowed.includes(status)) {
      return fail('VALIDATION_ERROR', 'Invalid application status filter', 400, requestId);
    }
    const rows = await sql`
      select
        a.id, a.relation_type, a.business_name, a.category_name, a.service_summary,
      ...
      where a.complex_id = ${String(manager.complex_id)}::uuid
        and (${status}::text is null or ${status} = 'all' or a.status = ${status})
      order by
        case a.status when 'pending' then 0 when 'changes_requested' then 1 else 2 end,
        a.created_at asc
    `;
  ...
  if (match && request.method === 'PATCH') {
    return patchApplication(request, sql, actor, match[1], requestId);
  }

  match = path.match(/^\/api\/v1\/admin\/complexes\/([^/]+)\/posts$/);
  if (match && request.method === 'POST') {
    return createPost(request, sql, actor, decodeURIComponent(match[1]), requestId);
  }

  match = path.match(/^\/api\/v1\/admin\/posts\/([0-9a-fA-F-]+)$/);
  if (match && request.method === 'PATCH') {
    return patchPost(request, sql, actor, match[1], requestId);
  }

  match = path.match(/^\/api\/v1\/admin\/complexes\/([^/]+)\/benefits$/);
  if (match && request.method === 'POST') {
    return createBenefit(request, sql, actor, decodeURIComponent(match[1]), requestId);
  }

  match = path.match(/^\/api\/v1\/admin\/benefits\/([0-9a-fA-F-]+)$/);
  if (match && request.method === 'PATCH') {
    return patchBenefit(request, sql, actor, match[1], requestId);
  }
