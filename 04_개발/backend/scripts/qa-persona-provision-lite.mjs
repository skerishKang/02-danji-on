 ...
 * name: a 401/400 on sign-in means the stored QA password no longer matches the
 * account, which is a provisioning/secret drift problem — not a product defect.
 */
export async function acquireAccount(frontendOrigin, apiOrigin, sql, account, fetchImpl = fetch) {
  ...
  const signupBody = await signup.text().catch(() => '');
  if (signup.ok) {
    created = true;
  } else if (signup.status === 422) {
    created = false;
  } else {
    throw new Error(`QA_PERSONA_LITE_SIGNUP_HTTP_${signup.status}:${account.name}:${safeBody(signupBody)}`);
  }

  const signIn = await fetchImpl(new URL('/api/auth/sign-in/email', frontendOrigin), {
    method: 'POST',
    headers: { accept: 'application/json', 'content-type': 'application/json', origin: frontendOrigin },
  ...
  if (!signIn.ok) {
    if (signIn.status === 401 || signIn.status === 400) {
      throw new Error(`QA_PERSONA_LITE_SIGNIN_PASSWORD_MISMATCH:${account.name}`);
    }
...
async function main() {
  ...
  const actors = [];
  for (const account of ACCOUNTS) actors.push(await acquireAccount(frontendOrigin, apiOrigin, sql, account));
