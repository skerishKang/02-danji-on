  ...
  const resultRows = await sql.query(`
    with input(slot, normalized_email, authority_level, scopes) as (
      values
        ...
        ('operational_1', $3::text, 'operator', array['benefit.manage','business.review','official-content.manage','resident_news.review']::text[]),
        ('operational_2', $4::text, 'operator', array['benefit.manage','business.review','official-content.manage','resident_news.review']::text[])
    ),
    guard as (
      select 1
