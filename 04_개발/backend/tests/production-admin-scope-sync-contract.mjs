...
assert.match(script,/active_grant_rows===38/,'post-sync readback must require 34 legacy rows plus four new current-scope rows');
assert.match(script,/principal_linked_active_grant_rows===38/,'all 38 active grants must remain principal linked after sync');
assert.match(script,/adoption_marked_active_grant_rows===38/,'all 38 active grants must retain the adoption lineage after sync');
