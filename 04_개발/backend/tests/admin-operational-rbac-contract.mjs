...
assert.doesNotMatch(authz, /complex_memberships/i, 'operational authority must never use legacy manager/admin membership');
assert.doesNotMatch(authz, /onboarding_support/i, 'day-to-day operational helper must not accept management-office support authority');
...
assert.doesNotMatch(admin, /requireManager|complex_memberships/i, 'new operational route layer must not use legacy manager/admin authorization');
