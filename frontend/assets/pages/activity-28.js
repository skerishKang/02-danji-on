...
const qp=new URLSearchParams(location.search),view=qp.get('view'),tabIndex=Math.max(0,Math.min(3,parseInt(qp.get('tab')||'0',10)||0)),activityApiBase=(globalThis.DanjionSession&&DanjionSession.danjionApiBase())||'',activityServerMode=Boolean(activityApiBase||(globalThis.DanjionSession&&typeof DanjionSession.isCanonicalProduction==='function'&&DanjionSession.isCanonicalProduction()));
...
document.addEventListener('click',function(ev){
 ...
 if(SCREEN==='messageDetail' && text.includes('공개 프로필')){ev.preventDefault();ev.stopImmediatePropagation();go(FILES.profile);return;}
