(function (global) {
  ...
  async function initAccountStrip(options = {}) {
    ...
    logoutStatus.className = 'danjion-account-logout-status';
    ...
    logout.addEventListener('click', async () => {
      ...
      if (!outcome.ok) {
        ...
        logout.textContent = '로그아웃';
        logoutStatus.textContent = '로그아웃하지 못했습니다. 잠시 후 다시 시도해 주세요.';
        logoutStatus.hidden = false;
        return;
      }
      location.href = 'index.html?intro=1';
    });
    actions.append(logout, logoutStatus);
