  ...
  function createCommunityBridge(options = {}) {
    ...
    return {
      ...
      async listComments(postId, options = {}) {
        ...
        const cursor = trimmedString(options.cursor);
        if (cursor) params.set('cursor', cursor);
        const result = await request(`${base}/posts/${encodeURIComponent(id)}/comments?${params.toString()}`);
      ...
      async listReplies(postId, parentCommentId) {
        ...
        const cursor = trimmedString(options.cursor);
        if (cursor) params.set('cursor', cursor);
        const result = await request(`${base}/posts/${encodeURIComponent(post.id)}/comments/${encodeURIComponent(parent.id)}/replies?${params.toString()}`);
