...

{
  ...
  assert.equal(s.danjionApiBase(), '', 'canonical Pages hostname must bind application API traffic to the same-origin Pages facade');
...
}
{
  ...
  assert.equal(s.danjionApiBase(), '', 'hostname comparison must be case-insensitive');
}
{
...
}
{
  ...
  assert.equal(s.danjionApiBase({ search: '', hostname: PRODUCTION_HOST }), '', 'resolver must accept an injected location for harnesses');
