...
const list = wiringBlock(page10, 'resident-news-list-server-wiring-20260911');
{
  ...
  assert.ok(list.block.includes('DanjionSession.danjionApiBase()') && /if\s*\(!apiBase&&!DanjionSession\.isCanonicalProduction\(\)\)\s*return;/.test(list.block),
    'list wiring must preserve canonical same-origin mode while keeping preview/local fallback fail-closed');
...
const detail = wiringBlock(page11, 'resident-news-detail-server-wiring-20260911');
{
  ...
  assert.ok(detail.block.includes('DanjionSession.danjionApiBase()') && /if\s*\(!apiBase&&!DanjionSession\.isCanonicalProduction\(\)\)\s*return;/.test(detail.block),
    'detail wiring must preserve canonical same-origin mode while keeping preview/local fallback fail-closed');
